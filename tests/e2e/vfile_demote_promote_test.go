...
func (s *VFileDemotePromoteTestSuite) TestSwarmRoleChange(c *C) {
	...
	log.Printf("Wait 60 seconds for new manager to be updated")
	time.Sleep(60 * time.Second)

	err = dockercli.DemoteNode(s.worker1, s.master)
	c.Assert(err, IsNil, Commentf("Failed to demote manager %s", s.master))

	log.Printf("Wait 60 seconds for new worker to be updated")
	time.Sleep(60 * time.Second)
	...
	log.Printf("Wait 60 seconds for volume status back to Ready")
	time.Sleep(60 * time.Second)
	...
	log.Printf("Wait 60 seconds for original manager to be updated")
	time.Sleep(60 * time.Second)

	err = dockercli.DemoteNode(s.master, s.worker1)
	c.Assert(err, IsNil, Commentf("Failed to reset worker role for %s", s.worker1))

	log.Printf("Wait 60 seconds for original worker to be updated")
	time.Sleep(60 * time.Second)
